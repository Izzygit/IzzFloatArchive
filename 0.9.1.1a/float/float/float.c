...
static void apply_inputtilt(data *d){ // Input Tiltback
	...
	float stickytilt_val = 100.0; // Value that defines where tilt will stick for both nose up and down. Can be made UI input later.
...
static void apply_torquetilt(data *d) {
	...
	if (d->state == RUNNING_WHEELSLIP) {
		...
		else if ((fabsf(d->acceleration) > 30) && (d->abs_erpm > 1000)) {
			d->torquetilt_interpolated *= 0.998;
			d->torquetilt_target *= 0.999;
			d->atr_interpolated *= 0.998;
			d->atr_target *= 0.999;
...
static void float_thd(void *arg) {
	...
	while (!VESC_IF->should_terminate()) {
		...
		case (RUNNING_UPSIDEDOWN):
			...
			if (fabsf(new_pid_value) > current_limit) { //Check for current limit
			...
			d->surge_startanglespeed = 1000000; //fmaxf(10, (float)d->float_conf.turntilt_start_erpm/10); 	//50	//Nose speed that can initiate a surge
			d->surge_difflimit = fmaxf(0.1, d->float_conf.turntilt_speed);				//2	//Pitch required at start angle speed to initiate surge
			d->surge_currentmargin = fmaxf(0.5, (float)d->float_conf.turntilt_erpm_boost/100);	//1.5	//Higher current margin ends surge later
			...
			if ((d->surge_diffcount >= d->surge_difflimit) && 		//Nose dip condition 
			     (SIGN(d->erpm) * d->proportional - 0.05 > 0) &&		//Minimum angle for acceleration
			...
			//Apply new pid value for current control
