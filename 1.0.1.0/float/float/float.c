...
static void check_traction(data *d){
	float wheelslip_margin = 6; 			// Acceleration difference that indicates the wheel is in traction
	float wheelslip_accelstart = 35; 		// Acceleration that starts wheelslip
	float wheelslip_accelend = 5; 			// Acceleration that indicates the initial wheelslip condition has ended
	// Use erpm factor to scale start accel so lower erpm requires more accel to start wheel slip. Has no effect above scaleerpm
	float wheelslip_scaleaccel = 5;
	float wheelslip_scaleerpm = 2000;
	float wheelslip_erpmfactor = wheelslip_scaleaccel - fminf(wheelslip_scaleaccel - 1, (wheelslip_scaleaccel -1) * fabsf( d->erpm / wheelslip_scaleerpm));
		...
		if (d->current_time - d->wheelslip_droptimeroff < 0.5) {		// Drop has ended recently
			...
			if (d->state == RUNNING_WHEELSLIP) { // skip this if we are already out of wheelslip to preserve debug values
				if (d->wheelslip_highaccelon1) {		
					if (SIGN(d->wheelslip_accelstartval) * d->accelavg1 < wheelslip_accelend) {	
				...
				} else if (fabsf(d->accelavg1) > wheelslip_margin) { 
	...
	if ((SIGN(d->motor_current) * d->accelavg1 > wheelslip_accelstart * wheelslip_erpmfactor) && 	// The wheel has broken free indicated by abnormally high acceleration in the direction of motor current
...
static void send_realtime_data(data *d){
	...
	buffer_append_float32_auto(send_buffer, d->erpm, &ind); //erpm current
