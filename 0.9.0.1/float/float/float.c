...
static void apply_inputtilt(data *d){ // Input Tiltback
	...
	//Check for Sticky Tilt Input Start
	if ((fabsf(d->inputtilt_interpolated) - fabsf(input_tiltback_target)) > 0){ // If the target is travelling toward or at the center
		if ((fabsf(d->inputtilt_interpolated)>  2.92) && (fabsf(d->inputtilt_interpolated) < 3.08)){
		//If we are close to 3, our sticky tilt value (this range allows for 120deg/s max. 832 hz is .144 step size, so we have .16 range.)
			if (SIGN(d->inputtilt_interpolated)==((d->throttle_val == 0.0) ? SIGN(d->inputtilt_interpolated) : SIGN(d->throttle_val))){   
		 	// If the throttle is at zero or not pushed to the opposite direction from sticky tilt value. 
				input_tiltback_target=3*SIGN(d->inputtilt_interpolated); // stay at 3, our sticky tilt value
			} else {
				input_tiltback_target=2.9*SIGN(d->inputtilt_interpolated); 
				// release target to below range, defined above, when throttle pushed in opposite direction                                             
			}                
                } 
        }
